...
业务 DOM
  ...
  -> 基于浏览器真实布局测量元素位置
  -> 自研分页算法计算 page slice
  -> 自研 SVG foreignObject + Canvas 合成视觉层，不使用 html2canvas
  -> Range.getClientRects() 计算逐字文字坐标
  -> 自研 PDF writer 组装页面图像、透明文字层、链接、书签
  -> Blob 自动下载，不打开打印框
```
...
- 高保真视觉输出：使用浏览器原生 DOM/CSS 结果作为视觉源，保留复杂样式、渐变、阴影、圆角、Flex/Grid、表格、SVG、图片等。
- 可复制文字：组件自行通过 `Range.getClientRects()` 提取文字位置，并在 PDF 内叠加透明 Unicode 文字层。
- 自动下载：通过 Blob URL 自动保存，不弹出系统打印框。
- 自研 PDF writer：不依赖 `jsPDF`，直接写 PDF objects、xref、image XObject、annotation、outline 和 Type3/ToUnicode 文字层。
...
- 本方案为了“原样还原”会把视觉层作为页面图像写入 PDF，同时叠加透明文字层；文字可复制/搜索，但 PDF 内可编辑矢量文本不是主目标。
