...
- 可复制文字：组件自行通过 `Range.getClientRects()` 提取文字位置，并在 PDF 页面最上层叠加 0 透明度的真实 Unicode 文字层。文字层使用 Type0/CIDFont、ToUnicode、ActualText 和零透明度 ExtGState，避免“只有 I-beam 但拖不出选区”的图片遮挡问题。
