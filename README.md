...
业务 DOM
  ...
  -> 自研 PDF writer 组装页面图像、可选文字层、链接、书签
  -> Blob 自动下载，不打开打印框
```
...
- 可复制文字：组件自行通过 `Range.getClientRects()` 提取文字位置，并在 PDF 内叠加真实可选 Unicode 文字层。
- 自动下载：通过 Blob URL 自动保存，不弹出系统打印框。
- 自研 PDF writer：不依赖 `jsPDF`，直接写 PDF objects、xref、image XObject、annotation、outline 和 嵌入式 Type0/CIDFont + ToUnicode/ActualText 文字层。
